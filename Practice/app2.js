document.addEventListener('DOMContentLoaded', () => {    //created DOM event listener all JS code will go in here
    ...
    function remove() {
        ...
        function movePlayerOne(e) {
            ...
            invaderId = setInterval(moveMotherShip, 3000)
            // setTimeout(function(){
           ...
           // creates a laser to shoot enemy ships 
            ...
            function shoot(e) {
                let laserId
                let currentEnemyLaserIndex = enemyInvaderMotherShip[0]

                //laser function
                function moveEnemyLaser() {
                    boxes[currentEnemyLaserIndex].classList.remove('laser')// this will remove laser befor adding new position
                    currentEnemyLaserIndex += screenWidth // laser will go back spaces = screenWidth (this makes laser go up 1)
                    boxes[currentEnemyLaserIndex].classList.add('laser')// this will add laser style to new box
                    if(boxes[currentEnemyLaserIndex].classList.contains('shooter')) { // because we moved we want to check to see if we hit anything
                        boxes[currentEnemyLaserIndex].classList.remove('laser')//remove laser style
                        boxes[currentEnemyLaserIndex].classList.remove('shooter')//remove invader style
                        boxes[currentEnemyLaserIndex].classList.add('boom') // create boom
                        const audio = new Audio("sound/sfx-boom.m4a");
                        audio.play();

                        setTimeout(() => boxes[currentEnemyLaserIndex].classList.remove('boom'), 250)// so the boom doesnt happen too fast set delay
                        clearInterval(laserId)
                        //This is adding up kills and displaying them at the
                        // const alienTakenDown = enemyInvaders.indexOf(currentLaserIndex)
                        // enemysKilled.push(alienTakenDown)
